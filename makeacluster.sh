...
time (
	if ! test -f "inventory.py" || ! ansible -i inventory.py -u $user -m ping all; then
		ansible-playbook --vault-password-file=vault_pass mapr_aws_bootstrap.yml
	...
	ansible-playbook -i inventory.py -u $user wait.yml
	ansible-playbook -i inventory.py -su $user --vault-password-file=vault_pass mapr_install.yml
)
